<?php
// 因为项目里面一个金额支付问题，被截取的位数总是会进1，
// 然后小数点截取方法使用了 substr 和 sprintf("%.9f",……) 这样的方法，
// 和使用floor()先乘再除一样，会丢失精度导致截取位数再往后一位为某个数字（比如9）时，
// 会进一位，遂干脆自己写一个，用纯字符串截取，毛事都没有了。

/** 
  * 截取浮点数的小数点位数，默认8位<br/>
  * @param double $double_val 浮点数
  * @param int $point_size 需要截取的小数点后位数
  * @return double 截取后的浮点数
   */
